...
const generateNewCard = (taskData) => `
  ...
  <div class="card-header  border-secondary d-flex justify-content-end gap-2">
...
const loadInitialCardData = () => {
  ...
  const getCardData = localStorage.getItem("data");
...
const deleteCard = (event) => {
  ...
  globalStore =  globalStore.filter((cardObject) =>{
    ...
    localStorage.setItem("data", JSON.stringify({cards:globalStore} ));
...
const saveChanges = () =>{
  ...
  localStorage.setItem("data", JSON.stringify({cards:globalStore} )); // Stores data in Key - Value pairs
